...
export async function getDeletedQuests(
  _req: Request,
  res: Response
) {
  try {
    const quests = await questsService.getDeletedQuests();
    ...
    console.error("Erreur récupération quêtes supprimées :", error);
...
export async function restoreQuest(
  ...
  try {
    ...
    const quest = await questsService.restoreQuest(questId);

    return res.status(200).json({
      success: true,
      data: { quest },
      message: "Quête restaurée avec succès.",
    });
  } catch (error) {
    console.error("Erreur restauration quête :", error);
...
export async function getUserQuests(
  ...
  try {
    ...
    const quests =
      await questsService.getUserQuests(
        userId,
        isAdminRequest
      );
