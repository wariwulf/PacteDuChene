...
export class QuestsRepository {
  async findAll() {
    return Quest.find({ deletedAt: null }).sort({ name: 1 });
  }

  async findDeleted() {
    return Quest.find({ deletedAt: { $ne: null } }).sort({
      deletedAt: -1,
      name: 1,
    });
  }

  async findByQuestId(questId: string, includeDeleted = false) {
  ...
  async findUserQuests(userId: string, includeDeleted = false) {
    ...
    const questIds = [
      ...new Set(userQuests.map((userQuest) => userQuest.questId)),
    ];
