...
export async function createAdmin(req:AuthenticatedRequest,res:Response){try{const body=req.body??{};const eventId=String(body.eventId??"").trim();if(!eventId)throw new Error("L'identifiant de l'événement est obligatoire.");const event=await clanEventsService.create({...buildData(body),eventId,status:(body.status??"PUBLISHED") as ClanEventStatus} as any,userId(req));return res.status(201).json({success:true,data:event});}catch(e){return res.status(400).json({success:false,message:message(e,"Impossible de créer l'événement.")});}}
