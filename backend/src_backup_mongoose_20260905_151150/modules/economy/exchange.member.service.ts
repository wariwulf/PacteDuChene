...
export async function exchangeCurrency(userId:string,from:string,to:string,amount:number){
 ...
 const updated=await User.findOneAndUpdate({_id:userId,status:{$ne:"DELETED"},[`economy.balances.${from}`]:{$gte:amount}},{$inc:{[`economy.balances.${from}`]:-amount,[`economy.balances.${to}`]:target}},{new:true});
