...
export class QuestsService {
  ...
  async deleteQuest(
    ...
    const deleted =
      await this.questsRepository.softDelete(
        questId
      );
  ...
  async restoreQuest(
    ...
    if (!existing.deletedAt) {
      throw new Error(
        "Cette quête n'est pas supprimée."
      );
    }

    const restored =
      await this.questsRepository.restore(
        questId
      );
  ...
  async reviewSubmission(
    ...
    if (status === "approved") {
      ...
      const questForTarget = await this.getQuest(submission.questId, true);
    ...
    const quest =
      await this.getQuest(submission.questId, true);
