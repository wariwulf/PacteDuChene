...
export class FactionsRepository {
  ...
  async upsert(config: FactionConfigData) { return Faction.findOneAndUpdate({ factionId: config.factionId } as any, { $set: config }, { upsert: true, new: true, setDefaultsOnInsert: true }).lean(); }
  async update(factionId: string, data: Partial<FactionConfigData>) { return Faction.findOneAndUpdate({ factionId } as any, { $set: data }, { new: true }).lean(); }
}
...
export class OrdersRepository {
  ...
  async attachDiscord(orderId: string, discordThreadId: string, discordMessageId: string) {
    return Order.findOneAndUpdate({ orderId }, { $set: { discordThreadId, discordMessageId } }, { new: true }).lean();
  }
}
