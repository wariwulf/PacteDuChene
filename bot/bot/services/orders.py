            ...
            try:
                ...
                for field in current.fields:
                    if field.name == "Statut":
                        current.set_field_at(current.fields.index(field), name="Statut", value=STATUS_LABELS.get(str(order.get("status")), str(order.get("status", "—"))), inline=field.inline)
                        break
                else:
                    current.add_field(name="Statut", value=STATUS_LABELS.get(str(order.get("status")), str(order.get("status", "—"))), inline=True)
                status = str(order.get("status"))

                # COMPLETED est l'état final utilisé par le système après livraison.
                # Une fois la commande terminée, le ticket Discord doit disparaître.
                if status in {"COMPLETED", "DELIVERED"}:
                    try:
                        await channel.delete(reason="Commande terminée — suppression automatique du ticket")
                        LOGGER.info(
                            "Ticket Discord supprimé pour la commande %s (statut=%s)",
                            order.get("orderId"),
                            status,
                        )
                    except discord.NotFound:
                        LOGGER.info(
                            "Le ticket Discord de la commande %s était déjà supprimé",
                            order.get("orderId"),
                        )
                    except (discord.Forbidden, discord.HTTPException):
                        LOGGER.exception(
                            "Impossible de supprimer le ticket Discord %s pour la commande %s",
                            thread_id,
                            order.get("orderId"),
                        )
                    continue
                ...
                LOGGER.info(
                    ...
                    order.get("orderId"),
                    STATUS_LABELS.get(status, str(order.get("status", "—"))),
                )
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
