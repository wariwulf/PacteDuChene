...
export default function EconomyPage() {
  ...
  const loadEconomy = useCallback(async () => {
    try {
      ...
      const [economyResponse, historyResponse] =
        await Promise.all([
          apiFetch<EconomyResponse>(
            `/economy/${userId}`
          ),
  ...
  const getBalance = (currencyId: CurrencyId) =>
    balances[currencyId];
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          <header className="mb-10">
            ...
            <p className="mt-3 text-gray-300">
              Consultez vos monnaies et suivez vos
              transactions économiques.
            </p>
          </header>

          <div className="rounded-2xl border border-amber-500/30 bg-[#07150f]/95 p-8 text-gray-200 shadow-2xl">
    ...
    return (
      <main className="relative min-h-screen overflow-hidden bg-[#07150f] px-6 py-12 text-white">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          ...
          <div className="rounded-2xl border border-red-400/40 bg-red-950/90 p-6 text-red-200 shadow-2xl">
            {error}
          </div>
          ...
          <button
            ...
            className="mt-4 rounded-lg bg-amber-600 px-5 py-3 font-semibold hover:bg-amber-500"
          >
            Réessayer
          </button>
        </div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <header className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          ...
          <button
            type="button"
            onClick={() => void loadEconomy()}
            className="rounded-lg border border-white/10 bg-[#062015]/90 px-5 py-3 font-semibold hover:bg-white/10"
          >
            ↻ Actualiser
          </button>
        </header>
        ...
        <section className="mb-10">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {CURRENCIES.map((currency) => (
              <article
                key={currency.id}
                className={`rounded-xl border p-6 shadow-xl transition ${currency.borderClass}`}
                style={{
                  backgroundColor: "rgba(6, 32, 21, 0.94)",
                  backdropFilter: "blur(2px)",
                  WebkitBackdropFilter: "blur(2px)",
                }}
              >
                <div
                  className={`mb-5 flex h-12 w-12 items-center justify-center rounded-lg text-2xl ${currency.iconClass}`}
                >
                  <CurrencyIcon currencyId={currency.id} />
                </div>
                ...
                <strong
                  className={`mt-4 block text-3xl font-bold ${currency.valueClass}`}
                >
                  {formatCurrency(
                    currency.id,
                    getBalance(currency.id)
                  )}
                </strong>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-xl border border-white/15 p-6 shadow-xl"
            style={{
              backgroundColor: "rgba(6, 32, 21, 0.94)",
              backdropFilter: "blur(2px)",
              WebkitBackdropFilter: "blur(2px)",
            ...
            <div className="divide-y divide-white/10">
              {CURRENCIES.map((currency) => (
                <div
                  ...
                  style={{
                    backgroundColor: "rgba(4, 24, 16, 0.78)",
                  }}
                >
                  <div>
          ...
          <article className="rounded-xl border border-white/15 p-6 shadow-xl"
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-sm text-gray-400">
                Suivez vos transactions économiques.
              </p>
            </div>

            {history.length === 0 ? (
              <div className="flex min-h-[220px] flex-col items-center justify-center rounded-lg border border-dashed border-white/10 bg-black/10 p-6 text-center">
                ...
                <p className="mt-2 max-w-md text-sm text-gray-500">
                  Les transactions liées aux quêtes,
                  récompenses, achats et échanges
                  apparaîtront ici.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {history.map((transaction) => {
                  ...
                  return (
                    <div
                      key={transaction._id}
                      className="flex items-center justify-between rounded-lg border border-white/10 bg-black/10 p-4"
                    >
