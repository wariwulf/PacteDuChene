...
const newStep = (index = 0): StepForm => ({
  ...
  imageUrl: "",
  difficulty: 1,
  // Toutes les étapes après la première sont déverrouillées par la précédente.
  requiresPreviousStep: index > 0,
...
function legacySteps(quest: QuestDefinition): StepForm[] {
  if (quest.steps?.length) {
    return quest.steps.map((step, index) => ({
      ...step,
      requiresPreviousStep: index === 0 ? false : step.requiresPreviousStep !== false,
    }));
  }
...
export default function QuestEditor({ questId }: Props) {
    ...
    async function load() {
      try {
        const [quests, achievementsResponse, currenciesResponse] = await Promise.all([
          getQuests(),
          fetch(`${API_URL}/achievements`, { credentials: "include", cache: "no-store" }).then(async (response) => {
            const payload = await response.json().catch(() => ({}));
            if (!response.ok || payload?.success === false) throw new Error(payload?.message || "Impossible de récupérer les exploits.");
            return payload;
          }),
          fetch(`${API_URL}/economy/currencies`, { credentials: "include", cache: "no-store" }).then(async (response) => {
            const payload = await response.json().catch(() => ({}));
            if (!response.ok || payload?.success === false) throw new Error(payload?.message || "Impossible de récupérer les monnaies.");
            return payload;
          }),
        ]);
  ...
  function moveStep(index: number, direction: -1 | 1) {
    setSteps((current) => {
      ...
      return copy.map((step, i) => ({ ...step, requiresPreviousStep: i > 0 }));
    });
  }
  ...
  function addObjective(stepIndex: number) {
    setSteps((current) => current.map((step, si) => {
      ...
      return { ...step, objectives: [...step.objectives, newObjective(stepIndex, index)] };
    }));
  }
  ...
  function selectStepImage(stepId: string, file: File | undefined) {
    if (!file) return;
    if (!file.type.startsWith("image/")) {
      setError("L'image d'une étape doit être un fichier image.");
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      setError("L'image d'une étape ne doit pas dépasser 10 Mo.");
      return;
    }
    setError("");
    setStepFiles((current) => ({ ...current, [stepId]: file }));
    setStepPreviews((current) => ({ ...current, [stepId]: URL.createObjectURL(file) }));
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name.trim()) return setError("Le nom de la quête est obligatoire.");
    if (!Number.isInteger(difficulty) || difficulty < 1 || difficulty > 5) return setError("La difficulté de la quête doit être comprise entre 1 et 5.");
    if (!steps.length) return setError("La quête doit posséder au moins une étape.");

    const slugify = (value: string) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const baseQuestId = editing && questIdValue.trim() ? questIdValue.trim() : slugify(name) || "quete";
    ...
    for (const [stepIndex, step] of steps.entries()) {
      if (!step.stepId.trim() || !step.name.trim()) return setError(`L'étape ${stepIndex + 1} doit posséder un nom.`);
      if (!Number.isInteger(step.difficulty) || step.difficulty < 1 || step.difficulty > 5) return setError(`La difficulté de l'étape ${stepIndex + 1} doit être comprise entre 1 et 5.`);
      if (!step.objectives.length) return setError(`L'étape ${stepIndex + 1} doit posséder au moins un objectif.`);
      for (const objective of step.objectives) {
        if (!objective.objectiveId.trim() || !objective.name.trim()) return setError("Chaque objectif doit posséder un nom.");
        if (objectiveIds.has(objective.objectiveId.trim())) return setError(`L'identifiant d'objectif "${objective.objectiveId}" est utilisé plusieurs fois.`);
        if (!Number.isInteger(objective.target) || objective.target <= 0) return setError("La cible de chaque objectif doit être supérieure à zéro.");
    ...
    const cleanedSteps = steps.map((step, index) => ({
      ...
      // La progression est toujours linéaire : chaque étape dépend de la précédente.
      requiresPreviousStep: index > 0,
    ...
    try {
      ...
      if (editing && questId) await updateQuest(questId, payload);
      else await createQuest(payload);

      const files = Object.entries(stepFiles);
      for (const [stepId, file] of files) {
        await uploadQuestStepImage(finalQuestId, stepId, file);
      }
  ...
  if (loading) return <main className="min-h-screen bg-[#173d2b] text-white"><div className="mx-auto max-w-6xl px-6 py-12 text-gray-400">Chargement...</div></main>;
    ...
    <main className="min-h-screen bg-[#173d2b] text-white">
      <div className="mx-auto max-w-6xl px-6 py-12">
        <header className="mb-10">
          ...
          <p className="mt-3 text-gray-300">Construisez la quête, son illustration, sa difficulté et son déroulement étape par étape.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-6">
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div><h2 className="text-2xl font-bold">Étapes</h2><p className="mt-1 text-sm text-gray-400">Une seule étape est accessible à la fois. La suivante se débloque automatiquement lorsque la précédente est entièrement terminée.</p></div>
            ...
            <div className="space-y-6">
              {steps.map((step, stepIndex) => {
                const preview = stepPreviews[step.stepId] || step.imageUrl;
                return (
                  <article key={`${step.stepId}-${stepIndex}`} className="rounded-2xl border border-white/10 bg-black/10 p-5">
                    <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                      <div><h3 className="text-lg font-bold text-amber-400">Étape {stepIndex + 1}</h3>{stepIndex > 0 && <p className="mt-1 text-xs text-gray-500">Débloquée après validation complète de l'étape {stepIndex}.</p>}</div>
                      <div className="flex gap-2"><button type="button" onClick={() => moveStep(stepIndex, -1)} disabled={stepIndex === 0} className="rounded bg-white/10 px-3 py-1.5 text-sm disabled:opacity-30">↑</button><button type="button" onClick={() => moveStep(stepIndex, 1)} disabled={stepIndex === steps.length - 1} className="rounded bg-white/10 px-3 py-1.5 text-sm disabled:opacity-30">↓</button>{steps.length > 1 && <button type="button" onClick={() => removeStep(stepIndex)} className="rounded bg-red-900/50 px-3 py-1.5 text-sm text-red-200">Supprimer</button>}</div>
                    </div>
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                      ...
                      <div className="md:col-span-2 rounded-xl border border-white/10 bg-black/20 p-4"><span className="mb-2 block text-sm font-semibold">Image de l'étape</span><input type="file" accept="image/*" onChange={(e) => selectStepImage(step.stepId, e.target.files?.[0])} className="block w-full text-sm text-gray-300 file:mr-4 file:rounded-lg file:border-0 file:bg-amber-600 file:px-4 file:py-2 file:font-semibold file:text-[#17130c] hover:file:bg-amber-500" /><p className="mt-2 text-xs text-gray-500">PNG, JPG, WEBP… · 10 Mo maximum. L'image est envoyée avec la quête.</p>{preview && <img src={preview} alt="Aperçu de l'étape" className="mt-4 h-44 w-full rounded-xl object-cover" />}</div>
                        ...
                        {step.objectives.map((objective, objectiveIndex) => (
                          <div key={`${objective.objectiveId}-${objectiveIndex}`} className="rounded-xl border border-white/10 bg-black/10 p-4">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                              ...
                              <label><span className="mb-1 block text-xs font-semibold text-gray-400">Cible d’événement <span className="font-normal text-gray-500">(optionnel)</span></span><input value={objective.eventTargetId ?? ""} onChange={(e) => updateObjective(stepIndex, objectiveIndex, "eventTargetId", e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2.5 outline-none focus:border-amber-500" /></label>
                              <label className="md:col-span-2 flex cursor-pointer items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-3 py-3"><input type="checkbox" checked={objective.requiresProof === true} onChange={(e) => updateObjective(stepIndex, objectiveIndex, "requiresProof", e.target.checked)} className="h-5 w-5 accent-amber-500" /><span><strong className="block text-sm">Preuve obligatoire</strong><small className="text-xs text-gray-500">L'objectif devra être validé par un administrateur avant de terminer la quête.</small></span></label>
          ...
          <section className="rounded-2xl border border-white/10 bg-black/20 p-6"><h2 className="mb-6 text-2xl font-bold">Récompenses</h2><div className="grid gap-5 md:grid-cols-2"><label><span className="mb-2 block text-sm font-semibold">XP</span><input type="number" min={0} value={rewardXp} onChange={(e) => setRewardXp(Number(e.target.value))} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3" /></label><label><span className="mb-2 block text-sm font-semibold">Quantité de monnaie</span><input type="number" min={0} value={rewardAmount} onChange={(e) => setRewardAmount(Number(e.target.value))} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3" /></label><label><span className="mb-2 block text-sm font-semibold">Monnaie</span><select value={rewardCurrencyId} onChange={(e) => setRewardCurrencyId(e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-amber-500"><option value="">Aucune monnaie</option>{availableCurrencies.map((currency) => <option key={currency.currencyId} value={currency.currencyId}>{currency.name} ({currency.currencyId})</option>)}</select></label><label><span className="mb-2 block text-sm font-semibold">Exploit attribué à la fin</span><select value={rewardAchievementId} onChange={(e) => setRewardAchievementId(e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-amber-500"><option value="">Aucun exploit</option>{availableAchievements.filter((achievement) => !achievement.linkedQuestId || achievement.linkedQuestId === questIdValue).map((achievement) => <option key={achievement.achievementId} value={achievement.achievementId}>{achievement.name} ({achievement.achievementId})</option>)}</select></label></div></section>

          <section className="rounded-2xl border border-white/10 bg-black/20 p-6"><label className="flex cursor-pointer items-center gap-3"><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-5 w-5 accent-amber-500" /><span><strong className="block">Quête active</strong><small className="text-gray-400">Les membres pourront la commencer.</small></span></label></section>

          <div className="flex justify-end gap-3"><button type="button" onClick={() => router.push("/administration/quetes")} className="rounded-lg bg-white/10 px-5 py-3 font-semibold hover:bg-white/20">Annuler</button><button type="submit" disabled={saving} className="rounded-lg bg-amber-600 px-6 py-3 font-semibold hover:bg-amber-500 disabled:opacity-50">{saving ? "Enregistrement..." : editing ? "Enregistrer les modifications" : "Créer la quête"}</button></div>
        </form>
      </div>
    </main>
  );
