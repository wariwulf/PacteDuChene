...
function getDiscordConfig() {
  ...
  return { token, guildId, circleRoleId };
}
...
/**
 * Synchronise le droit d'administration issu du rôle Discord
 * « Membre du Cercle ».
 *
 * Cette synchronisation ne peut promouvoir qu'un PLAYER vers ADMIN.
 * Elle ne rétrograde jamais un ADMIN/OWNER attribué manuellement.
 */
export async function syncCircleRoleForUser(memberId: string, discordId?: string) {
  ...
  if (user.role === UserRole.OWNER) {
    return { role: user.role, promoted: false, owner: true };
  }

  const { guildId, circleRoleId } = getDiscordConfig();

  if (!circleRoleId) {
    throw new Error(
      "DISCORD_ROLE_MEMBRE_DU_CERCLE n'est pas configuré dans le .env du backend."
    );
  }

  const link = await getDiscordLink(memberId);
  const effectiveDiscordId = discordId?.trim() || link?.discordId;

  if (!effectiveDiscordId) {
    return { role: user.role, promoted: false, inGuild: false };
  }

  try {
    const guildMember = await discordFetch<DiscordGuildMember>(
      `/guilds/${encodeURIComponent(guildId)}/members/${encodeURIComponent(
        effectiveDiscordId
      )}`
    );

    const hasCircleRole = (guildMember.roles || []).includes(circleRoleId);

    if (hasCircleRole && user.role === UserRole.PLAYER) {
      user.role = UserRole.ADMIN;
      user.discord.lastSyncAt = new Date();
      await user.save();
      return { role: user.role, promoted: true, inGuild: true, hasCircleRole: true };
    }
    ...
    // Ne pas bloquer une connexion à cause d'une indisponibilité temporaire
    // de Discord. La synchronisation sera retentée à la prochaine connexion.
