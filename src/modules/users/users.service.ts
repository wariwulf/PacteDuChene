...
function canManageRole(actorRole: ActorRole, targetRole: UserRole) {
  // Le propriétaire peut attribuer n'importe quel rôle.
  if (actorRole === "OWNER") return true;

  // L'administrateur peut uniquement attribuer/révoquer le rôle de modérateur
  // (ou laisser/rendre un membre PLAYER). Il ne peut jamais créer un ADMIN
  // ou un OWNER.
  if (actorRole === "ADMIN") {
    return targetRole === UserRole.PLAYER || targetRole === UserRole.MODERATOR;
  }

  // Les modérateurs et joueurs ne peuvent pas modifier les rôles.
  return false;
}
...
export class UsersService {
  ...
  async update(
    ...
    // Toute modification de rôle suit une politique distincte du reste du
    // profil. Le rôle ne peut être changé que par OWNER ou ADMIN, avec les
    // limites définies ci-dessus.
    if (input.role !== undefined && input.role !== user.role) {
      if (actorRole === "MODERATOR" || actorRole === "PLAYER") {
        throw new Error("Seul un administrateur ou le propriétaire peut modifier les rôles.");
      }
      ...
      if (!canManageRole(actorRole, nextRole)) {
        throw new Error("Vous ne pouvez attribuer que le rôle Modérateur ou Joueur.");
      }

      // Un ADMIN ne peut ni créer un second ADMIN, ni toucher à OWNER.
      if (actorRole === "ADMIN" && (user.role === UserRole.ADMIN || nextRole === UserRole.ADMIN || nextRole === UserRole.OWNER)) {
        throw new Error("Seul le propriétaire peut gérer les rôles Administrateur et Propriétaire.");
      }
    }
